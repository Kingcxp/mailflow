# Build log — MailFlow v0.1.0 reconstruction

Honest record of what was actually executed and verified while rebuilding
MailFlow stage by stage (one commit per stage). Every commit passed
`git diff --check`; gates were re-run after each stage.

Environment: Windows 11, Python 3.11.4, uv 0.11.2, git 2.55. The workspace
resolved to Python 3.11 (tzdata added as a Core Windows dependency so
`ZoneInfo` works).

## Stage log

| Stage | Commit | Executed verification |
| ----- | ------ | --------------------- |
| 00 bootstrap | `chore: bootstrap uv workspace...` | `uv lock`, `compileall tools` |
| 01 domain | `feat(core): define normalized mail...` | `pytest tests/unit/test_core.py` (15) |
| 02 config | `feat(core): add typed runtime configuration...` | unit tests (30) |
| 03 contracts/registry | `feat(core): introduce typed component contracts...` | `compileall`, `ruff check` |
| 04 pluggy | `feat(core): add entry-point plugin discovery...` | `mypy packages/mailflow-core`, `pyright` |
| 05 events/llm/pipeline | `feat(core): add event bus, llm fallback routing...` | unit tests (48), mypy, pyright strict |
| 06 logging | `feat(core): add queue-based rich logging...` | unit tests (52) |
| 07 i18n | `feat(core): add english chinese and external json localization...` | unit tests (63) |
| 08 runtime | `feat(core): add bounded async runtime...` | unit tests (73) |
| 09 service | `feat(core): expose embeddable service facade...` | unit tests (84) |
| 10 commands | `feat(core): add transport-neutral management command router...` | unit tests (97) |
| 11 testkit/fake | `test: add reusable fake mail llm and notifier components...` | mypy+pyright over packages/plugins, unit tests |
| 12 sqlite | `feat(storage): add sqlite persistence...` | integration tests (11) |
| 13 openai backend | `feat(llm): add configurable openai-compatible...` | integration tests (15) |
| 14 processors | `feat(processors): classify mail urgency replies...` | unit tests incl. critical exam |
| 15 notifier + e2e | `test: cover public service startup...` | full suite (124) |
| 16 bundled + cli | `feat(cli): add rich standalone host...` | full suite (126); `mailflow --help`, `command help`, `doctor`, `snapshot --json` |
| 17 tui | `feat(tui): add mail action runtime log and settings interface...` | full suite (127) incl. headless TUI pilot |
| 18 configs/i18n | `docs(config): add complete runtime...` | `config-check` on both configs; `mail list/show`, `lang set zh-CN` against dev config |
| 19 build/exe | `build: add workspace wheel and nuitka...` | `uv build --all-packages` (11 wheels); Nuitka standalone compiled 909 C files, exe verified: `command help`, `config-check`, `mail list` |
| 20 docs | `docs: document mailflow architecture...` | `make docs`, `make check` |

## Final gates (executed, not assumed)

```bash
uv run ruff check .            # pass
uv run ruff format --check .   # pass
uv run mypy packages plugins tools   # pass (strict)
uv run pyright                 # pass (strict; CLI file uses # pyright: basic)
uv run pytest -q               # 127 passed
uv run python tools/check_docs.py  # pass
uv build --all-packages        # pass (11 wheels)
```

## Manual smoke tests (executed)

- `uv run mailflow --help`, `command "help"`, `doctor`, `snapshot --json`
- `command "mail list"` / `mail show` / `lang set zh-CN` with
  `configs/development.toml` (LLM endpoint absent → graceful failure notes,
  rules result kept)
- frozen `dist/frozen_entry.dist/frozen_entry.exe`: `command "help"`,
  `config-check -c configs/development.toml`, `command "mail list"` — all
  pass; locale JSON + app.tcss bundled.

## Not executed

- Real provider integrations (IMAP/Gmail/Outlook) — explicitly out of
  scope for the 0.1.0 baseline (planned provider phase).
- Network LLM calls in tests: never performed (httpx monkeypatched).

## Phase 2 (post-tag review & features)

| Commit | Change | Executed verification |
| ------ | ------ | --------------------- |
| `fix(core): retry storage saves...` | save retries, broader secret redaction, coverage for all packages | `make coverage` (86%), full suite |
| `feat(core): remind on timed actions...` | early + day-of reminders, persisted fired state | reminder unit tests (17 runtime tests) |
| `feat(core): manage every config option...` | config list/get/set, TUI config table, comment-preserving TOML patching | `config set` CLI smoke on a commented file (comments preserved) |
| `feat(core): plugin marketplace...` | repo/market/install commands, TUI Market tab, `mailflow-repo` sibling repo | end-to-end install of the webhook sample via `uv pip`; uninstalled after |

Final phase-2 gate: 144 tests passed, mypy/pyright strict clean, ruff clean.

## Executed after the initial log

- `make exe-onefile` equivalent: `tools/build_exe.py --mode onefile`
  completed; `dist/frozen_entry.exe` verified (`plugin list`,
  `config-check -c configs/development.toml`).
- `uv run mailflow tui -c configs/development.toml` launched and rendered
  (header, tabs, search placeholder) in this environment.
- Interactive `mailflow shell` pipelined `help mail`, `mail list`,
  `lang get`, `exit` successfully.

## Marketplace v2 round (post v0.2.0)

| Change | Verification |
| ------ | ------------ |
| Per-plugin folder layout: root `index.json` (categories only) + `category/<id>/plugin.json` (markdown readme) + source; `_list_plugin_dirs` over file://, GitHub contents API, INDEX.json fallback | CLI e2e: `repo add` local + `market list` (5) + `market show`; unit/e2e fixtures migrated; 152 tests |
| Plugin i18n: `MarketPlugin.descriptions` / `readmes`; `description_for` / `readme_for` pick the active app language (CLI list/show/search + TUI table/detail); search matches translations | localization tests (zh-CN readme/description, en fallback) |
| Rich markdown spans: render via rich Markdown with `<span style="color:…">` support (sentinel markers → span styles), bold/strike/code/quote/bullets/headings; bounded and merged with overlapping styles | span-color + boundedness unit tests |
| Plugin template generator (`mailflow.plugin_template`): 5 category stubs producing complete loadable plugins | 14 template tests; e2e: scaffold → temp venv install → entry-point discovery → `build_registry` instantiation → processor `process()` ran |
| TUI new-plugin wizard (`PluginScaffoldScreen`): DirectoryTree folder pick + subfolder checkbox/name + template-category Select | headless pilot test scaffolds into tmp_path |
| `mailflow-repo` documentation: docs/ (getting started, metadata ref, categories, per-category guides, localization, validation), per-category README + INDEX.json, per-plugin README, README links | repo pushed; validator `--all` passes 5/5 |
| Plugin PR validation workflow (`validate-plugins.yml`) + `tools/validate_plugin.py`: changed-plugin detection (unchanged plugins skipped), metadata/install/entry-point/registration checks + real processor run | local run of the script against all 5 plugins: `ok: 5 plugin(s) valid, loadable and runnable` |
| MailFlow CI workflow (`ci.yml`): uv sync, ruff check/format, mypy, pyright, pytest, docs gate on push + PR (ubuntu + windows) | workflow committed |

## Bot-export round (post v0.2.0)

| Change | Verification |
| ------ | ------------ |
| `BOT_EXPORTER` component kind + `registrar.add_bot_exporter`/`bot_exporter_factory`; `mailflow.bot_export` (`BotExportContext`/`BotExportResult`, `available_frameworks`, `export_bot_plugin`) | pyright/mypy strict clean; unit tests (registry + export + exporters) |
| `plugins/mailflow-export-nonebot` + `plugins/mailflow-export-astrbot` (framework plugin generators embedding resolved config + deps) | workspace smoke: both frameworks export and config round-trips; scratch-venv install → entry-point discovery → factory instantiation OK |
| `bot_exporter` scaffold category in `mailflow.plugin_template` (wizard + template) | scaffolded module compiles, imports, reports BOT_EXPORTER kind, registers framework id |
| CLI `mailflow export --framework <id> --output <dir>` + `make bot-plugin*` targets | CLI export runs end-to-end on the dev config |
| TUI `BotExportScreen` wizard (framework select + DirectoryTree + subfolder) wired to the Market tab Export button | headless pilot e2e test exports into tmp_path |
| `mailflow-repo`: `bot_exporter` category (2 plugins), docs/bot-exporter.md, validator support | `validate_plugin.py --all` passes 5/5 existing + 2/2 new (against local core; scratch-env core install requires the pushed core) |
| README.zh-CN.md + README bot-export docs; MAILFLOW_FROM_ZERO.md removed, references cleaned | docs gate OK (35 mandatory documents) |

## User action items round (post bot-export round)

| Change | Verification |
| ------ | ------------ |
| `action add|delete` commands (summary + --due/--type/--notes, local-zone parsing) and `service.add_action/delete_action`; `list_actions` merges mail-derived + user items sorted by due time | command unit tests (add/list/show/delete, validation, mail-derived not deletable) |
| StorageBackend protocol + SQLite `custom_actions` table + MemoryStorage/FakeStorage fakes | sqlite roundtrip/overwrite/delete + restart-persistence tests |
| Reminder scheduler fires for user items too (`_fire_reminder` refactor; record=None payload) | runtime test: user item fires once, record is None, no re-fire |
| i18n keys (en/zh-CN) for add/delete/source-user; README commands rows | i18n parity test green |

## Letter replies + formatting round (post user-action round)

| Change | Verification |
| ------ | ------------ |
| `mailflow.letters`: cn/en formal-letter templates (auto date, right-aligned signature block), lightweight markup dialect (`**bold** *italic* <right> <center>`, blank-line paragraphs), `html_to_text` | 13 unit tests (structure, skeleton, escaping, round-trip, passthrough, alignment) |
| `service.create_letter_draft(mail_id, language)` (config-timezone today); commands `reply compose <id> <cn\|en>`, `reply edit` markup conversion, `reply show` plain text | command tests: compose cn/en + unknown template, edit markup, show without tags |
| TUI ReplyModal: template buttons + format toolbar (bold/italic/left/center/right via TextArea selection/line ops), dialog layout in app.tcss | e2e: apply cn template (structure + right align + date), bold selection, align line, save persists |
| `plugins/mailflow-notify-telegram` + marketplace copy, notifier docs/INDEX/README | integration test (urllib monkeypatch: URL/body, skip-without-credentials); `validate_plugin.py` ok 1/1 |
| i18n keys (reply.*, tui.reply_*) en/zh-CN; README command rows; changelog | i18n parity test green |

## Chat/updates/template round (post letter round)

| Change | Verification |
| ------ | ------------ |
| Account-independent mail identity + in-flight/storage dedup in the runtime | dedup tests: cross-account forward stored/notified once, refetch skipped, digest identity |
| Paginated chat listings (mail/action/plugin/help, 10/page), --query, unique-prefix ids, wrap-friendly rows | command tests: pagination/query/prefix |
| `feedback` command → rolling guidelines injected into LLM analysis (ProcessingContext) | pipeline injection tests + command tests |
| Daily 08:00 digest event (today/upcoming counts + items) once per day | runtime digest tests (fires once, pre-hour silent) |
| `mailflow.updates`: GitHub release check, plugin marketplace version check, update-source gating, `update` commands, daily auto-update loop, `general.auto_update` | updates unit tests (release/plugin/local-source gating/commands/loop) |
| Local plugin installs: `plugin install <path>` + TUI file-tree installer (detect_plugin_folders) | command tests single/batch/empty |
| Chat bridge in exported nonebot/astrbot templates (mailflow prefix, chunked replies, digest paging); marketplace copies synced | templates compile-checked; export tests |
| `mailflow.plugin_api` declarative decorators; scaffold templates generate this style with dev dependency group + uv install docs | plugin_api tests; all six template categories compile/load/register |
| docs/development/deployment.md (3 platforms) + overview decorator style; docs gate now 37 | docs gate OK |

## Processor rework + built-in mail/LLM plugins round

| Change | Verification |
| ------ | ------------ |
| Core classification built in: `mailflow.processors` (RulesProcessor, LLMImportanceProcessor, `register_builtin_processors`, enhancer aggregation); default chain `rules@10, llm-importance@20`; `LLMEnhancer` protocol + `ComponentKind.LLM_ENHANCER` + `add_llm_enhancer`/`llm_enhancer` decorator | make check green (259 tests); template factories instantiate for all 7 categories |
| Removed `mailflow-processor-rules` / `mailflow-processor-llm-importance` plugins; bundled/pyroot deps, uv.sources, mypy/pyright paths, tests migrated | make check green (plugin_id now "mailflow-core") |
| `plugins/mailflow-llm-anthropic`: Messages API backend (x-api-key, sanitized errors, bounded retries, `_parse` content blocks) | pyright 0 errors, ruff clean, mypy clean; FakeClient test verifies system/messages payload |
| `plugins/mailflow-mail-imap`: stdlib imaplib/smtplib, presets qq/163/outlook/gmail/generic, MIME→MailMessage pure `parse_mime`, SMTP replies, per-poll dedup | integration tests: MIME text/html, SMTP login/send, IMAP poll + dedup; make check 264 tests |
| Both plugins registered in `mailflow-bundled` + workspace (uv.sources, mypy_path, pyright extraPaths); TestBundledRegistration updated | make check green |
| Marketplace: mail_source/mailflow-mail-imap + llm_backend/mailflow-llm-anthropic copies, llm_enhancer category, processor docs, README/INDEX; validator now installs declared third-party deps | `validate_plugin.py --all` ok 10/10 |
| Docs: deployment (presets table + anthropic config), plugin overview/processor (LLMEnhancer), plugin-system (LLM_ENHANCER kind, 8 bundled plugins), README en/zh-CN, CHANGELOG | docs gate OK |
## TUI fixes round (post market round)

| Change | Verification |
| ------ | ------------ |
| Textual 8 button regression: `height: 3` + tall border overflowed 3-row containers (labels collapsed to 0), and the stock variant palette draws on ansi_default (black-on-black). Flat high-contrast variants (`$primary/$success/$warning/$error/$surface-lighten-1` + dark text) with `height: auto`; container heights relaxed | probe: all buttons render labels, height > 0; `test_market_buttons_rendered_with_labels` |
| Full-screen plugin detail modal (title, metadata author/updated/homepage, markdown readme, install/uninstall/enable/disable) on row select; row highlight previews in the pane | `test_market_detail_shows_author_and_updated` (modal assertion) |
| Config edit modal: select a config row → form (value + cancel/save) → `service.set_config_value` persists and table refreshes; list-valued options show a TOML hint | `test_config_edit_modal_saves_value` (timezone → Asia/Shanghai persisted) |
| `general.summary_language`: pins LLM summary/reason/reply language; empty follows the interface language; injected into the built-in llm-importance user message | `TestSummaryLanguage` (instruction injected / absent without config) |
| Config descriptions localized via `config.desc.<key>` keys (en + zh-CN) with english fallback | TUI e2e checks description renders (not the raw key) |
| `cursor_type: row` on every DataTable — row selection events never fired under the default `cell` cursor | all table interaction e2e green |
| make check full suite | 268 tests + docs gate OK |

## Settings editor + mailbox history round (post TUI fixes round)

Review of both repositories first (docs + all sources), then one commit per
fix batch.

| Change | Verification |
| ------ | ------------ |
| **Bug:** the TUI and CLI `run`/`shell` never forwarded `--config` to `start_service`, so `service.config_path` was `None` and every persisting action failed with "no config file loaded" | `test_config_path_forwarded_to_start_service` (runner stub asserts the kwarg) |
| **Bug:** `write_config` wrote resolved secrets into the TOML file (a `${VAR}` value and an `api_key_env`-resolved key both became plaintext `api_key`). `load_config` now records placeholder paths in `env_placeholders` (excluded from serialization) and `write_config` restores them; env-backed keys are blanked | `TestConfigPersistenceSecrets` (placeholder restored, env key absent from the written file) |
| **Bug:** `_patch_section_line` fell back to a whole-file scan when the section was missing, so `general.language` rewrote `[i18n].language`. Now strictly section-scoped, returning None so the caller rewrites | `TestPatchConfigValue` (3 cases incl. the cross-section regression) |
| **Bug:** the app subscribed to `mail.processed` while the runtime emits `mailflow.mail.processed`, so panes never auto-refreshed | `test_processed_mail_event_refreshes_panes`; confirmed it fails when the name is reverted |
| **Bug:** the IMAP source advanced `_last_uid` before fetching, so a transient FETCH error dropped that mail forever. The mark advances after parsing and the loop breaks on the first failure | `test_failed_fetch_does_not_skip_the_mail` (uid 2 fails, then is delivered) |
| `mailflow.settings`: OptionSpec/EditorKind from the pydantic field type, per-plugin sections, apply/reset, list-entry add/update/remove/move, LLM chain derivation, SettingsError naming the option | `tests/unit/test_settings.py` (27 tests: editors, coercion, range/validator errors, reset, entry CRUD, chain reorder, reference scrubbing) |
| Service facade: `settings_sections`, `settings_option`, `set_setting`, `reset_setting`, `add/update/remove/move_config_entry`, comment-preserving persist | exercised through the TUI e2e settings tests |
| Optional history capability: `HistoryCapableSource` protocol, `service.history_accounts/fetch_history/process_mail/is_mail_known`, runtime `process_mail_now` returning the record (None on duplicate), IMAP + fake implementations | `TestMailboxHistory` (5 tests) + `test_fetch_history_is_newest_first_and_independent` (browsing leaves the poll water-mark alone) |
| TUI Settings tab rewritten: search box, section sidebar, one card per option (description, default, typed editor, Save/Restore default), ListEditScreen, EntryFormScreen; render serialized under a lock (duplicate widget ids otherwise) | `test_settings_card_saves_resets_and_rejects`, `test_tui_compose_and_data` (sidebar + search filter + no secret in the rendering) |
| New Mailboxes and LLMs tabs | `test_mailbox_history_analyzes_selected_mail` (known mail skipped, unseen mail analyzed with a summary), `test_llm_tab_orders_the_fallback_chain` (move up → default/fallback rederived) |
| Market refresh no longer blocks: exclusive worker for the fetch, local render for search/category; install state cached | market e2e tests green; measured fetch of the official repo at ~8s off the UI thread |
| Buttons: explicit variants everywhere, opaque base `Button` background, Back button in the repository dialog | `test_market_repos_screen` asserts the Back button label, non-default variant and that it closes the screen |
| 70 localized option descriptions (en + zh-CN) via `tools/gen_option_descriptions.py`; 8 English keys that held Chinese text fixed; `t()` skips formatting without params | i18n parity test; `config.desc.<key>` resolved for all 70 keys in both packs |
| Marketplace repo: every plugin.json readme rewritten with option tables, urgency mappings and troubleshooting (en + zh-CN) | `validate_plugin.py` metadata checks 10/10; all readmes loaded through `PluginMarket` from a `file://` copy |
| Docs: configuration/overview (settings editor, secret write-back), architecture/tui (new tabs), architecture/domain-and-mail (history capability), agent/invariants (+12 contracts), agent/module-map, pipeline/overview/embedding event names, README en+zh, CHANGELOG | docs gate OK (37 documents) |
| make check, round 1 | 310 tests + lint + format + mypy + pyright + docs gate green |
| **Round 1 TUI smoke** (fake source, commented config): all 8 tabs activate; sidebar 6 sections, 14 cards; search narrows to one option; save rewrites `workers = 3` with the file comment intact; an invalid value warns "expected a whole number" and leaves the value; restore-default returns 2; history loads 2 mails | executed against `/tmp/smoke.toml` |
| **Bug found by that smoke run:** required entry fields rendered `PydanticUndefined`, so an LLM added without an id was stored as "PydanticUndefined" and poisoned the chain. Fixed (`_field_default` + blank-required rejection) | `test_required_fields_have_no_sentinel_default`, `test_blank_required_text_is_rejected`; re-ran the form flow: prefill `''`, save refused with "llm_id is required", form stays open |
| **Round 2** with a zh-CN interface, `Asia/Shanghai` and an `${R2_TOKEN}` secret: CLI `config-check` OK, `mail list`/`config set` answer in Chinese, the patched file keeps its comment and the placeholder (no plaintext key); TUI tab labels 邮件/邮箱/待办/大模型/设置, Chinese option descriptions, secret never rendered, plugin-owned section lists `processors[0].*`, `api_key` field is a password input, Back closes the form, history marks 已解析 and analyzing a known mail is a no-op | executed against `/tmp/r2.toml` |
| make check, round 2 | 313 tests + lint + format + mypy + pyright + docs gate green |

## Documentation sync round (post settings/mailbox round)

Goal: make every document match the code so a later agent or contributor can
trust it, and make that property enforceable instead of aspirational.

| Change | Verification |
| ------ | ------------ |
| **Stale docs found by cross-checking:** `module-map.md` and `change-playbook.md` still listed `mailflow-processor-rules` / `mailflow-processor-llm-importance` as plugins (deleted when the processors moved into core), `llm.md` pointed at the deleted plugin path, `deployment.md` documented a `make doctor` target that never existed, and `tests.md` described a test layout missing 9 of 19 files | each corrected against the code; `make doctor` → `uv run mailflow doctor` verified by running it |
| `change-playbook.md`: "Add a processor" rewritten around the real options (LLMEnhancer for bounded customization vs a new processor plugin, and the fact that registering an existing component id replaces the built-in); new "Add a configurable option" procedure; mail-source entry documents the optional history capability | procedures walked against `processors.py`, `plugin_api.py`, `settings.py` |
| `embedding.md`: the full service surface (58 methods) grouped by task, plus a table of all 17 events with their payloads | every documented payload diffed against the `events.emit(...)` call sites — 16/16 match |
| `tests.md`: real per-file layout and counts; `i18n.md`: the `t()` no-params rule, the namespace inventory and where `config.desc` comes from; `overview.md`/`llm.md`/`mail-source.md`: settings, history capability, Anthropic backend, built-in processors, LLMEnhancer | docs gate + suite green |
| **Latent bug found while auditing the locale packs:** 21 keys were spelled as literal dotted keys (`"tui.btn_cancel"`) instead of nested objects, and one of them duplicated an existing nested key — both flatten to the same lookup path, so iteration order decided the winner and editing the other did nothing. All 21 folded into proper nesting; packs sorted for reviewable diffs | `test_no_duplicate_lookup_paths_in_builtin_packs` (re-introducing the duplicate makes it fail), plus parity/no-Chinese-in-en/config.desc-coverage tests; 458 keys per pack, parity holds |
| Removed two leftover `plugins/mailflow-processor-*` directories that held only orphaned `.pyc` files and made deleted plugins look installed | `plugins/` now lists exactly the eight bundled plugins |
| **Docs gate upgraded** from "files exist" to "files exist and are accurate": `tools/check_docs.py` now cross-checks quoted repo paths, `make` targets, `mailflow.*` event names, `service.<method>(` references and `mailflow-*` plugin ids against the code. `CHANGELOG.md` and `BUILD_LOG.md` are exempt as historical records; marketplace ids are only checked when the sibling checkout exists | all five checks proven by injecting one bad reference of each class and confirming the specific failure message; gate reports "35 documents cross-checked" |
| Invariants 32–35 added (locale key uniqueness, `config.desc` coverage, docs name real things); quality.md documents the gate; agent/README.md explains what it can and cannot catch | `make check` |
| Configs: `development.toml`/`example.toml` comments updated — `imap` is built in with presets (they still told users to look for a mail adapter in the marketplace), plus the LLM tab's order-is-the-chain rule | `mailflow config-check` on both |
| make check | 318 tests + lint + format + mypy + pyright + docs gate green |

## Mail processing failure-isolation round

| Change | Verification |
| ------ | ------------ |
| Rate-limited mail diagnosis: no MIME parse failures were found; historical failures were `llm-importance` deadlines. `RulesProcessor` now stops an advertisement at `ad`, avoiding unnecessary LLM requests, and an `asyncio.wait_for` expiry persists its configured deadline rather than `failed: ` | Regression tests first reproduced both defects, then `tests/unit/test_pipeline.py`, `tests/unit/test_llm_processor.py` and `tests/unit/test_failed_mails.py` passed; IMAP regression selection passed 11 tests (23 deselected) |
| Full quality gate | `make check`: Ruff clean; 176 formatted files; mypy 93 source files clean; pyright 0 errors; pytest 490 passed (1 third-party deprecation warning); docs gate cross-checked 37 documents |

## Malformed MIME retention round

| Change | Verification |
| ------ | ------------ |
| IMAP ingestion emits a stable best-effort `MailMessage` for an otherwise unparseable RFC-822 payload; `parse_error` becomes a persisted, sanitized `source-parser` failure note instead of silently losing the message or blocking later UIDs. | `make check`: Ruff clean; 178 files formatted; mypy 94 source files clean; pyright 0 errors; pytest 494 passed (1 third-party deprecation warning); docs gate cross-checked 37 documents. |

## WeChatPadPro automatic-deployment round

| Change | Verification |
| ------ | ------------ |
| Guided WeChatPadPro deployment accepts Compose v2 and classic v1, starts a dormant Docker Desktop, installs Docker Engine and a compatible Compose package separately on apt hosts, allocates unreserved API/webhook ports, and maps the Linux container to MailFlow's host webhook bridge. | `tests/unit/test_wechatpadpro_provisioner.py`: Compose-v1 command form, fresh and dormant Docker Desktop startup, port collision, host mapping, and complete webhook body handling. |
| A guided notifier saves `base_url` and loads its QR-minted credential from private per-instance metadata instead of copying it into editable notifier config. | Unit notifier send assertion verifies the managed key is used for `/Msg/SendTxt`; 8 focused tests passed. |
| Full quality gate after the change. | `make check`: Ruff clean; 179 files formatted; mypy 94 source files clean; pyright 0 errors; pytest 502 passed (1 third-party deprecation warning); docs gate cross-checked 37 documents. |

## TUI localization and interaction round

| Change | Verification |
| ------ | ------------ |
| Mail, actions, notifications, settings, dialogs, splash, remote login, and footer shortcuts now resolve visible labels through `service.t(...)`; persisted urgency/action values, booleans, defaults, timestamps, and empty states render in the active language and configured timezone. The remote adapter follows the server `language.changed` event. | `uv run pytest tests/e2e/test_tui.py tests/e2e/test_settings_forms.py tests/e2e/test_remote_tui.py tests/unit/test_ask_correct.py tests/unit/test_notify_feed.py`: 35 passed; `uv run pytest tests/unit/test_i18n.py tests/unit/test_locale_completeness.py`: 17 passed. |
| Responsive mail controls use a full filter row and equal-width two-row action grid; every standalone form is a centered bordered panel. Ask & Correct serializes in-flight questions, preserves history ordering, and returns localized safe failures. | Interactive `uv run mailflow tui --config tmp-tui-smoke.toml` smoke: zh-CN header, tabs, search, empty state, filters, and action grid rendered correctly; temporary config/database removed. |
| Full quality gate after the change. | `make check`: Ruff clean; 179 files formatted; mypy 94 source files clean; pyright 0 errors; pytest 504 passed (1 third-party deprecation warning); docs gate cross-checked 37 documents. |

## Smart search progress and relevance round

| Change | Verification |
| ------ | ------------ |
| Smart search sends opaque per-batch candidate aliases to the LLM, keeps scored matches ordered by relevance, reports warmup/batch status without spinner overwrites, and keeps unavailable batches explicitly incomplete. The TUI runs its request outside the button message handler, so Cancel remains usable and restores the full mailbox. | `make format && uv run pytest tests/unit/test_service.py::TestSmartSearch tests/unit/test_commands.py::TestCommandRouter tests/e2e/test_tui.py::test_smart_search_keeps_real_progress_and_relevance_order -q`: 41 passed. |
| Full quality gate after the change. | `make format && make check`: Ruff clean; 179 files formatted; mypy 94 source files clean; pyright 0 errors; pytest 509 passed (1 third-party deprecation warning); docs gate cross-checked 37 documents. |

## Mobile chat command round

| Change | Verification |
| ------ | ------------ |
| Command replies are page-fitted below both 1,600 UTF-8 bytes and 1,600 UTF-16 units without discarding text; ordinary long replies get localized ordered labels, while sectioned help/example content remains grouped. `config list` now exposes every option for transport paging, and `reply create` accepts the stable `#n` displayed by `mail list`. | `uv run pytest tests/unit/test_i18n.py tests/unit/test_locale_completeness.py tests/unit/test_commands.py tests/unit/test_bot_export.py tests/unit/test_wechatpadpro_provisioner.py tests/integration/test_chat_chain.py -q`: 105 passed. The Unicode regression reconstructs a 1,800-code-point CJK/emoji response exactly and checks each returned page against both limits. |
| WeChatPadPro ACKs the webhook before a slow dispatch, tracks/cancels outstanding forwards on shutdown, sends every BotServer reply page, and loads the managed gateway key once for the sequence. Exported NoneBot and AstrBot templates invoke the same page fitter and compile after export. | Focused webhook and exporter coverage is included in the 105-test command above. |
| Initial `make check` exposed six strict-pyright unknown-JSON boundary errors in the new paths; explicit typed boundaries fixed them. | Rerun `make check`: Ruff clean; 179 files formatted; mypy 94 source files clean; pyright 0 errors; pytest 513 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Seminar confirmation and Ask & Correct rendering round

| Change | Verification |
| ------ | ------------ |
| Seminar discovery remains review-only: candidates are persisted separately from action items, can be edited/rejected, require explicit future timezone-aware confirmation, preserve source-mail/location/URL, and use a stable timed-event key despite model title drift. | `uv run pytest -q tests/unit/test_ask_correct.py tests/unit/test_service.py -k "ask_correct or seminar" tests/e2e/test_settings_forms.py -k "ask_correct or seminar"`: 13 passed. |
| Ask & Correct conversation entries render user and assistant Markdown while the source mail stays escaped; the LLM request runs in a cancellable Textual worker and gates Send until its ordered response is rendered. | Focused TUI worker/Markdown test included in the 13-test command above. |
| Full quality gate after the changes. | `make check`: Ruff clean; 180 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 521 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Failed-analysis disclosure round

| Change | Verification |
| ------ | ------------ |
| Subject continuity is now explicitly distinct from analysis success: pipeline and empty LLM summaries set `summary_is_fallback`; legacy pipeline notes remain recognizable. The TUI and Ask & Correct show the localized failure/unavailable state instead of presenting the subject as an LLM summary, retain genuine reasons, and preserve the original body. | `uv run pytest tests/unit/test_core.py tests/unit/test_pipeline.py tests/unit/test_llm_processor.py tests/unit/test_failed_mails.py tests/e2e/test_settings_forms.py tests/unit/test_i18n.py tests/unit/test_locale_completeness.py -q`: 124 passed. The actual TUI launch rendered the localized failed-analysis state, unavailable summary/reason, and original body for a persisted failed mail. |
| Re-analysis progress/completion moved to a durable operation-status widget so `mailflow.mail.processed` refreshes cannot erase it; bulk re-analysis now dispatches before selected-mail-only actions, so it really works with no row selected. | The focused TUI regression explicitly clears selection, then waits for and observes the localized completion state after a real re-analysis event. |
| Full quality gate after the change. | `make check`: Ruff clean; 180 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 524 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Gateway deployment lifecycle hardening round

| Change | Verification |
| ------ | ------------ |
| Retrying or re-logging-in a gateway cancels and joins its previous supervisor, preserves its current bridge options, and carries one non-persisted progress channel through install and start. Cancellation or failure now stops partial gateways and persists a truthful stopped/error state; a failed Compose stop reaches the caller rather than claiming success. | `uv run pytest tests/unit/test_gateway.py tests/unit/test_wechatpadpro_provisioner.py -q`: 29 passed. |
| WeChatPadPro Compose resource names include a bounded instance-id digest, preventing different valid notifier ids with equivalent legacy slugs from sharing containers, networks, or volumes. The guide treats Escape as Cancel and cleans the active deployment. | Focused regression cases cover resource collision isolation, failed cleanup propagation, and Escape cancellation. |
| Full quality gate after the change. | `make format && make check`: Ruff clean; 180 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 530 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Smart search precision round

| Change | Verification |
| ------ | ------------ |
| The smart-search prompt and service now treat scored candidates below 40 as non-matches. The same floor applies before streaming batch previews, so irrelevant low-confidence rows never flicker into the TUI; 40–59 remains available for useful weak matches and legacy unscored arrays remain compatible. | `uv run ruff format packages/mailflow-core/src/mailflow/service.py tests/unit/test_service.py tests/e2e/test_tui.py && uv run pytest tests/unit/test_service.py::TestSmartSearch tests/unit/test_commands.py::TestCommandRouter tests/e2e/test_tui.py::test_smart_search_keeps_real_progress_and_relevance_order -q`: 43 passed. |
| Full quality gate after the change. | `make check`: Ruff clean; 180 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 531 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## TUI error presentation and remount safety round

| Change | Verification |
| ------ | ------------ |
| Generic TUI backend failures now use localized `common.error` framing; configured passwords, tokens, LLM keys, and header values are redacted before rendering, and Rich markup in diagnostics is escaped. Every TUI path that had rendered a raw exception string now uses the shared presentation helper. | `uv run pytest tests/unit/test_tui_labels.py tests/unit/test_i18n.py tests/unit/test_locale_completeness.py tests/unit/test_notify_feed.py tests/e2e/test_tui.py tests/e2e/test_settings_forms.py tests/e2e/test_remote_tui.py -q`: 52 passed. A fresh 140×50 Textual `run_test` smoke rendered `错误：save [test] failed: ***` in Notifications; its temporary database and SVG were removed. |
| A concurrent language switch and app shutdown no longer lets a queued pane remount mount into a detached `TabPane`. | The Notifications regression switches to zh-CN, renders the credential-redacted failure, then closes immediately; it failed before the attachment-lifecycle guard and passes now. |
| Full quality gate after the change. | `make check`: Ruff clean; 181 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 532 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Smart action and reported TUI regression round

| Change | Verification |
| ------ | ------------ |
| The Mail tab's "Smart find" control is one **Smart action** entry: the instruction box filters mail as you type, and the button sends the same text to `service.smart_action`, which classifies the intent. `search` keeps ranked matching; `schedule_seminar` matches the mails announcing an attendable event, schedules the proposals with a usable future start through `import_seminar`, and hands the rest (`needs_review`) to the review form instead of guessing. Discovery is no longer a standalone button: the seminar scan UI, its spinner and its six locale keys are gone, and `SmartActionResult` carries the intent, matches, created entries and review leftovers. | `uv run pytest tests/unit/test_service.py::TestSmartAction tests/unit/test_service.py::TestSmartSearch tests/unit/test_service.py::TestSeminarDiscovery -q`: 19 passed. `uv run pytest tests/e2e/test_tui.py -q -k smart_action`: 2 passed, including a real pilot run that scheduled `Research colloquium` with its source-mail link, window and location, and asserted the Actions tab shows no review control once nothing is pending. |
| Mail tab controls are back to the layout the dropdowns and buttons had before: one control row holding the three selects plus a two-row button container (refresh/trash/Ask & Correct, then reply/re-analyze/re-analyze-failed) with equal-width buttons; the filter row and spacer are gone. The Actions tab gained an explicit **Edit** button for the row under the cursor (custom todos and imported seminars) beside Add/Delete. | A headless 120×32 pilot reports the controls as `Horizontal`, the first select and the first button row both at row 25, three buttons in row 1, and the mail detail still 19 rows tall; the Actions toolbar reads Add todo / Edit / Seminar proposals (0)… / Delete selected. `uv run pytest tests/e2e/test_settings_forms.py -q`: 17 passed, including the new create → edit → re-render → delete round trip. |
| Ask & Correct's input row is one bordered line: input, Send and Close share a row of height 3 instead of the buttons floating on the input's first line. | The same pilot reports input/send/close regions `(27, 3)` each with `aligned: True`. |
| The mail detail always shows the stored summary and reason; a storage fallback is labelled ("not generated — the source subject stands in") instead of hiding the content, and a message-less processor failure is persisted by its exception type so a note can no longer read `failed: ` with nothing after it (pipeline, LLM router). | `uv run pytest tests/unit/test_pipeline.py tests/unit/test_logs_categories.py tests/unit/test_i18n.py tests/unit/test_locale_completeness.py -q`: 45 passed, including `test_message_less_failure_still_names_the_cause`. `uv run pytest tests/e2e/test_settings_forms.py -q`: 17 passed with the updated fallback contract (subject visible + labelled, real reason still visible). |
| The Logs tab can no longer render a raw category key: `tui.logs_cat_llm/parse/notify/storage/system` were missing, so LLM lines showed `tui.logs_cat_llm`. Both packs now define all seven categories and a unit test derives the required keys from the pane's own mapping. | Injected log lines in a zh-CN pilot render `[通知]  HTTP 500: Internal Server Error` and a source filter reading 全部来源/聊天机器人/大模型/邮件/通知/解析/存储/系统 (no raw keys). `tests/unit/test_logs_categories.py::test_every_category_is_translated_in_both_packs` fails the old packs. |
| Full quality gate after the change. | `make check`: Ruff clean; 181 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 540 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Mail control width, LLM edit and first-token timeout round

| Change | Verification |
| ------ | ------------ |
| The Mail tab's bottom controls are the layout they had before the regression: the three dropdowns take two thirds of the row (`2fr` each) and the two-row button container the remaining third (`3fr`) with equal-width `1fr` buttons and a one-column gap, instead of the equal-quarters split that squeezed long labels together. | A headless 120×32 pilot reports the container 39 columns wide with 12-column buttons starting at 80/93/106 (a one-column gap); at 140×45 the container is 45 columns with 14-column buttons. In zh-CN every label fits its content region (重析失败邮件 needs 12 cells, has 12 at 120×32 and 14 at 140×45). |
| Editing an `[[llms]]` entry no longer fails with "llm … does not match any configured llm": the settings editor re-derives `default`/`fallback` from the list order and rewrites every processor binding (and cross-LLM fallback) *before* validating, so renaming or editing an id validates against the repaired config. | `uv run pytest tests/unit/test_settings.py -q`: 43 passed, including `test_renaming_an_llm_rewrites_every_reference`, `test_renaming_the_bound_llm_follows_the_processor` and `test_adding_an_llm_extends_the_chain`. `uv run pytest tests/e2e/test_settings_forms.py -q -k editing_an_llm_id`: the real `EntryFormScreen` edit renames `beta` → `beta-2`, the service persists it, and the written `cfg.toml` contains the renamed id with no stale `"beta"` reference. |
| `[[llms]] timeout_seconds` defaults to 120 s (was 60) and `[[processors]] timeout_seconds` to 120 s (was 30): both bound the wait for the model's first token plus the whole answer, and 30 s made every mail of a cold local model time out into a subject fallback. The descriptions in both packs say so, the Internal ceilings for seminar/smart batches went 180 s → 300 s, and the LLM form's Test probe uses a 60 s request budget with a 120 s wait. | `uv run python tools/gen_option_descriptions.py`: 74 descriptions regenerated for both packs (it also restored `general.command_prefix` and `general.hourly_summary`, which the previous run had dropped). `uv run pytest tests/unit/test_settings.py::TestTimeouts tests/unit/test_i18n.py -q`: passed, including the every-option-has-a-description gate. |
| Full quality gate after the change. | `make check`: Ruff clean; 181 files formatted; mypy 95 source files clean; pyright 0 errors/0 warnings; pytest 546 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Mail bulk actions, recipient profile and ad calibration round

| Change | Verification |
| ------ | ------------ |
| The Mail tab's control block gained a third button row: **Clear expired mail**, **Re-analyze all** and **Mail preferences**. Both bulk actions state the real count in a confirmation dialog first; nothing runs straight from a click, and cancellation is proved to run nothing. | `uv run pytest tests/e2e/test_tui.py -q -k bulk_buttons`: a pilot run asserts the dialog names the expired-mail count, that confirming removes exactly those ids and leaves a mail with a future deadline, that the purged mail sits in the trash and restores, and that the re-analyze dialog reports the current stored count. |
| "Expired" is now a defined, documented rule in the service (`list_expired_mails`): no future `action_items`, urgency ad or info, `manual_urgency` untouched, and at least 24 h old when it is an ad so the message being read right now is never swept away; urgent/important mail is never proposed. The purge is the same recoverable trash move the per-mail Delete uses. | `uv run pytest tests/unit/test_service.py -q`: 40 passed, including `TestExpiredMail` (old ad and past info purged; fresh ad, future info, undated info, urgent, important and manually re-classified mail kept) and the no-op case. |
| A recipient profile (`feedback.profile`, `service.user_profile()`/`set_user_profile()`) is captured by a form and threaded into `ProcessingContext.user_profile`, so every analysis and every smart action/search carries it. It travels in the *user* message as data about the recipient, never in the system prompt, so a mail cannot impersonate it as an instruction. | `uv run pytest tests/unit/test_service.py tests/unit/test_llm_processor.py -q`: profile round trip (trimmed), profile reaching the processor context, profile appearing in the smart-action request, and the prompt/`_build_messages` profile contract all pass. |
| The analysis prompt was retuned: `ad` is now defined as the smallest bucket (unsolicited sales, spam, unusable system chatter) and mass-mailed institutional notices, newsletters with a date, recruitment invitations and workshop/library announcements are named as `info`; the old "and any bulk mail" clause is gone, and the feedback notes may not be used to turn an announcement into `ad`. | `uv run pytest tests/unit/test_llm_processor.py -q`: 21 passed, including the prompt-contract tests asserting the ad/bulk rule, the profile rule and the profile being appended to the user message. |
| An independent adversarial review of the destructive path found four majors and seven minors; every finding that changed behaviour is fixed and covered: the purge deletes exactly the ids the dialog listed and re-reads/re-checks each record immediately before its own delete (a manual classification or re-analysis landing while the dialog is open wins); a mail is never "expired" when its analysis failed, fell back to the subject or never ran; a `due_end` window that is still running counts as live; schedule entries that point back at a mail (`mail_id`) count as its obligations; a user-initiated delete refreshes the trash retention window so a mail already in the trash cannot expire right after the user was told it is restorable; both dialogs dismiss at most once (a second dismissal used to pop the whole screen stack and end the session); the bulk workers run on the app so a pane remount cannot cancel a half-finished purge, and a failure (e.g. remote mode, which does not implement the call) is reported instead of doing nothing. | `uv run pytest tests/unit/test_service.py -q`: 46 passed (new `TestExpiredMailGuards`: failed/fallback analysis, live schedule entry, still-running window, re-check before delete, exact-id scope, refreshed window). `uv run pytest tests/integration/test_plugins.py -q -k window`: the real SQLite backend refreshes the retention stamp for a user delete while keeping the preserved timestamp for the automatic path. `uv run pytest tests/e2e/test_tui.py -q -k bulk_buttons`: the dialog count matches what is deleted and a button-then-Escape double dismissal leaves the app running. |
| Full quality gate after the change. | `make check`: Ruff clean; 183 files formatted; mypy 97 source files clean; pyright 0 errors/0 warnings; pytest 562 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Classification quality round: keyword hits, feedback pollution, live tuning

Diagnosed against the user's own mailbox (a SQLite snapshot of `data/mailflow.db` re-analyzed with their configured LLM; their file was never written to). After their full re-analysis the live snapshot held **35 ad / 13 info / 1 urgent and zero important**, and no mail had an action item at all.

| Change | Verification |
| ------ | ------------ |
| **The keyword pre-filter no longer decides.** `RulesProcessor` returned `STOP` + `ad` on a single keyword hit, and every bulk mailing — university notices, lecture invitations, newsletters — carries "unsubscribe"/"click here" in its footer, so exactly that mail was classed `ad` without ever reaching the model. A hit is now a hint that contributes an `ad` overlay and *continues* the chain (the overlay survives an LLM failure); one strong word (`promotion`/`sale`/`discount`/`advertisement`/`limited offer`/`act now`) hints, footer boilerplate needs two independent hits, and a user-configured `advertising_keywords` list is still taken at face value. Important-sender entries contribute `important` and also continue, so the model can raise them. | Live A/B on the same real mails: `INVITATION: Envisioning Islands – Public Lecture` `ad` → **info**; `Invitation to PAIR Distinguished Lecture` `ad` → **important** (registration + deadline); `招募健康青少年` `ad` → **info**. `uv run pytest tests/unit/test_pipeline.py -q`: 23 passed, including the new `test_promotional_hint_does_not_stop_the_chain` and `test_bulk_footer_boilerplate_is_not_promotional`. |
| **The prompt now makes `important` reachable and bounds the feedback notes.** Mail is judged by what the recipient must do: anything to act on, answer or track is at least `important` (a stated deadline or required action does so even inside a bulk notice, newsletter or invitation), `info` is optional/FYI, `ad` is unusable mail only; being bulk or automated is explicitly not a reason to choose `info`/`ad`; the model must name the reason before choosing `ad`; every stated deadline must yield an action item. Feedback notes are framed as narrow, kind-scoped preferences that can never override a deadline or a required action. | Live: the safety notice yielded **urgent** with an extracted action item (`清理…个人物品`, 09-11 09:00), the registration reminder and the seminar-report submission reminder yielded **important**, the catering survey with a 27 Sep deadline yielded **important** — where the old run had zero action items and no `important` at all. `uv run pytest tests/unit/test_llm_processor.py -q`: 24 passed (prompt contract: action/deadline ⇒ important, feedback cannot override rules, English-only urgency tokens). |
| **Repeated feedback no longer reads as an absolute rule.** The user's store held the *same sentence 20 times* ("这是营销广告，永远归为 ad") because repeats were appended blindly; the prompt showed all twenty, and 17 of 49 stored reasons cited it. `record_feedback` now replaces a repeat instead of appending (`_FEEDBACK_WINDOW` distinct notes), and the prompt collapses duplicates at render time so already-polluted stores are fixed too. | `uv run pytest tests/unit/test_service.py tests/unit/test_llm_processor.py -q`: 84 passed, including `TestFeedbackWindow` (25 identical notes store as one) and `TestGuidelineRendering` (a 20× duplicate plus one distinct note renders as two lines). |
| **Translated urgency tokens are accepted.** `parse_urgency` only knew English synonyms, so a model answering in Chinese (`重要`, `紧急`, `广告`, …) silently became `info`. | `TestUrgencySynonyms` covers 重要/紧急/广告/推广/信息 plus the English variants; unknown values still default to `info`. |
| Full quality gate after the change. | `make check`: Ruff clean; 102 files formatted; mypy 97 source files clean; pyright 0 errors/0 warnings; pytest 579 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Interruption, LLM fallback, timeout migration and detail spacing round

| Change | Verification |
| ------ | ------------ |
| The bulk re-analysis is interruptible: while it runs, the button itself becomes **Stop re-analysis** (error variant) and the loop checks the stop flag between mails, finishing the mail in flight and reporting `Re-analysis stopped: {count} of {total} mail(s) analyzed`. The running flag is separate from the stop request, so a second press stops the job instead of starting another one. | `uv run pytest tests/e2e/test_tui.py -q -k stopped_while_running`: a pilot starts the run with a service whose `process_mail` blocks, asserts the label flips to Stop, presses it, releases the in-flight mail, and asserts the run ended after exactly one mail with the partial count in the status line. |
| A failing primary LLM now routes to the next named LLM. `_bind_llm_processor` only wrote `fallback_llms` when the primary binding itself changed, so adding a second LLM left the analysis processor without fallbacks and a primary failure simply failed the mail. The chain is now re-derived whenever it is missing or points at ids that no longer exist, while an explicitly configured list is preserved. | Live probe with a primary backend that raises and a second that answers: the router retried the primary and then served the mail from `working` (`backend: working`, urgency `important`, both processor notes `ok` instead of `failed`). `uv run pytest tests/unit/test_service.py -q`: 61 passed, including the new binding tests (second LLM becomes reachable, deletion clears the binding, explicit fallbacks kept). |
| Timeouts are editable where the user is: the LLM entry form now edits `timeout_seconds` and `max_retries` (they were hidden by the "minimal fields" list, which is why raising them meant hand-editing TOML). | The form exposes `#field-timeout-seconds` / `#field-max-retries`; `uv run pytest tests/e2e/test_settings_forms.py -q`: 18 passed. |
| Configs written by earlier versions carry the old shipped defaults (60 s per LLM request, 30 s per processor) and are migrated on load: `mailflow.config.migrate_legacy_timeouts` raises exactly those two values to the current defaults and logs the change; a value the user picked deliberately is left alone. `start_service` writes the migrated config back, so the file and every settings screen show the value in effect. | Their real `configs/development.toml` (60/30) loads as 120/120 and the file itself is rewritten at startup — verified against a copy, then by `uv run pytest tests/unit/test_settings.py -q -k TimeoutMigration` (legacy defaults raised, deliberate 15 s preserved, fresh config needs nothing). |
| The mail detail no longer leaves blank rows above the summary: empty status statics are hidden instead of holding a row each. | The fallback probe reports `analysis_status_display: False`, `operation_status_display: False` with the summary at the top of the pane. |
| Full quality gate after the change. | `make check`: Ruff clean; 102 files formatted; mypy 97 source files clean; pyright 0 errors/0 warnings; pytest 586 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Fast list edits and preserved analysis on failure round

| Change | Verification |
| ------ | ------------ |
| Adding, editing, deleting or reordering a list entry returns immediately. `MailFlowRuntime.reconfigure` restarted every source task on every settings change, and because a source parked in a blocking connect cannot be cancelled it waited up to five seconds before the edit appeared — for edits that cannot affect sources at all (an LLM timeout, a notifier target, a reordered chain). Sources are now restarted only when the accounts, their options or an adapter's class changed; plugin (un)loads still force a restart, and an explicitly configured change to accounts keeps the previous behaviour. | Measured against their real config with a source whose connect blocks in a thread: editing an LLM entry takes **0.00 s** (the teardown path is what used to hold the UI). `uv run pytest tests/unit/test_runtime.py -q -k SourcesSurvive`: 3 passed — unchanged accounts keep the same source instance and never call `close()`, a changed account restarts it, and `restart_sources=True` still forces it. |
| **A failed re-analysis no longer replaces a working analysis.** When the new run times out, is rate-limited or answers unusably (subject fallback or a failed processor note) and the mail already had a completed analysis, the previous summary, reason, urgency, action items and manual override are kept and the new failure is appended to the note trail; the TUI counts that mail as failed with the model's own error instead of as a success. | `uv run pytest tests/unit/test_runtime.py -q`: 33 passed, including `test_failed_run_keeps_the_previous_analysis` (previous summary/urgency/action items survive, failure note appended), `test_successful_run_replaces_the_previous_analysis` and `test_first_analysis_failure_is_stored_as_a_fallback`. The TUI batch reports per-mail failures from the stored notes. |
| Full quality gate after the change. | `make check`: Ruff clean; 102 files formatted; mypy 97 source files clean; pyright 0 errors/0 warnings; pytest 592 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## Analysis latency, timeout semantics and bulk-control correctness round

Diagnosed against the user's own mailbox and their configured LLM (a DB/config snapshot; their files were never written to).

| Finding | Evidence | Fix |
| ------- | -------- | --- |
| The configured timeout was not a wall-clock deadline: the transport timeout applies per read/write chunk, and a processor timeout was retried, so a 30 s setting still showed "analysing" long after 30 s. | Live probe against their endpoint: every call reported `request exceeded 120s`; with `retries=1` a 30 s processor timeout could hold a mail for 60 s. | The router wraps every request in `asyncio.wait_for(config.timeout_seconds)` and reports `request exceeded Ns` (then tries the next named LLM); a processor timeout is terminal and is no longer retried. `uv run pytest tests/unit/test_pipeline.py -q -k TimeoutSemantics`: a timeout is attempted once and returns within the deadline, while other failures still retry. |
| A cold locally hosted model paid its load time out of the first mail's budget. | Their first live call exceeded 180 s while later ones were fast (and the endpoint then returned HTTP 429). | The runtime warms the first LLM up once at startup in the background (`LLMRouterImpl.warmup`, one 4-token completion, 300 s allowance). |
| The analysis prompt had grown to 6.7 KB/106 lines (the original was ~2.5 KB), inflating input tokens and giving a small model more room to drift or truncate. | Measured `len(SYSTEM_PROMPT)` = 6755 before this round. | Rewritten to 3.9 KB/66 lines with the same behavioural rules (action/deadline ⇒ important, ad is unusable mail only, feedback notes are scoped, action items from every stated obligation). |
| Re-analyze-all was the only stop-aware bulk button; `_reparse_batch` swallowed `CancelledError`; and the `_reparse_running` flag could latch, so after cancelling the dialog re-analysis could never start again. | The user reported exactly that; both stop tests failed against the previous state machine. | One lifecycle for both long buttons (`_start_bulk_reparse` always restores them), "running" derived from the worker rather than a flag, cancellation propagated, the other button parks while a run is in flight, and a re-entrancy guard that defeated its own first run was removed. `uv run pytest tests/e2e/test_tui.py -q -k "stopped_while_running or bulk_reparse_stop"`: 2 passed (including "cancel the dialog, then start again"). |
| Moving an LLM up reset the cursor to row 0 (the table rebuild + highlight event), so consecutive moves were impossible; a rejected move also moved the cursor. | The user reported the cursor reset after every move. | The pane restores the cursor to the selected row after a rebuild (with the highlight suppressed), selects the entry's new position before a move, restores it when the move is rejected, keeps a neighbour selected after a delete, and reports at the chain ends. `uv run pytest tests/e2e/test_tui.py -q -k llm_move_up`. |
| Their endpoint could not answer at all during the diagnosis: a 4-token warm-up failed immediately and four real calls each exceeded 120 s. | Live probe output; earlier in the same session the endpoint answered the first call after >180 s and then returned `HTTP 429` for every later one. | Reported to the user as an endpoint-side issue (rate limit or an overloaded local model); the app now fails fast with the model's own error, keeps the previous analysis, and never hangs the UI indefinitely. |
| Full quality gate after the change. | `make check`: Ruff clean; 102 files formatted; mypy 97 source files clean; pyright 0 errors/0 warnings; pytest 607 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

## LLM binding, cancel semantics and end-to-end TUI walkthrough round

| Change | Verification |
| ------ | ------------ |
| **Analysis now uses the model the user pinned.** Their new endpoint was first in the chain (`default=true`) while the `llm-importance` processor still pointed at the old AMD model, so every mail first waited out its 120 s timeout before falling back — which read as "the configured timeout never fires". The binding now follows the chain head (the list order *is* the routing policy) and rotates the previous primary into the first fallback; the processor's own timeout is raised to cover the bound model's request budget (+15 s). | Their real config: binding moved from `DeepSeek V4 Flash (AMD Radeon)` to `DeepSeek V4.1 Flash (Dumb Cat)` with the old one as fallback and the processor timeout 135 s. Live run against the new endpoint: **6/6 mails analyzed in 11.7–34.8 s** (previously four of four hit the 120 s ceiling), with sensible levels — the seminar-report notice (deadline 2 Dec) → `important` + 2 action items, the Micro Fund final call → `important`, optional lectures/activities → `info`, the mooncake promo → `ad`. |
| Every re-analysis button is a cancel control, and cancelling is immediate: the worker is cancelled instead of only setting a flag, so an in-flight request no longer holds the UI until its own timeout. One run at a time (the others park), and the lifecycle always restores all three buttons. | `uv run pytest tests/e2e/test_tui.py -q -k "bulk_reparse_stop or stopped_while_running"`: 2 passed — single-mail cancel restores every button promptly, the bulk dialog can be cancelled and re-run, and a stopped run always hands the controls back. Fixing the dispatch also removed a leaked coroutine ("was never awaited"). |
| Bulk re-analysis analyzes three mails concurrently. | Same tests plus the walkthrough; the per-mail progress line reports completions. |
| The startup warm-up honours the configured request budget (was a flat 300 s allowance) so nothing at startup runs far past the timeout the user set. | `make check` clean; the warm-up is bounded by `max(60, llm.timeout_seconds)`. |
| **A full TUI walkthrough test** (`tests/e2e/test_tui_walkthrough.py`) drives the real app: table fills, detail pane shows summary/reason/body with no blank rows, typing filters and clearing restores, single-mail re-analyze offers cancel and cancels cleanly, bulk re-analyze confirms/cancels/re-runs, clear-expired asks and cancels without deleting, the preferences form saves, smart action returns matched rows, every tab mounts, the LLM chain reorders with the cursor following, and the log viewer filters. Any unhandled handler exception fails it. | `uv run pytest tests/e2e/test_tui_walkthrough.py -q`: 1 passed. |
| The **real TUI** (runner + service + app, not a pilot) was launched against a copy of their config and left running for two minutes: it rendered the mail table with the 重要/信息/广告 mix, the detail pane (摘要/原因/原文内容) and the three control rows, with their live mailbox attached. | `hub` process `realtui` (ready on the MailFlow banner, 2 m04s uptime, then terminated externally — Textual quits on Ctrl+Q, which the PTY helper cannot send). |
| Full quality gate after the change. | `make check`: Ruff clean; 103 files formatted; mypy 97 source files clean; pyright 0 errors/0 warnings; pytest 610 passed (1 existing third-party deprecation warning); docs gate cross-checked 37 documents. |

### Schedule expiry sweep (2026-09-18)

| Command | Result |
| ------- | ------ |
| `uv run pytest tests/unit/test_actions_hide.py -q` | 7 passed (4 new expiry regressions) |
| `uv run pytest tests/e2e -q` | 57 passed |
| `make check` | see below |

`service.purge_expired_actions()` retires schedule entries that ended more than
a day ago, driven by an hourly `_schedule_expiry_loop` in the service (plus once
at startup). Mail-derived entries are dismissed by the same natural key a manual
delete records, so re-analysis cannot resurrect them; custom todos and imported
seminars are deleted from the custom-action store. The window end (`due_end`)
decides "ended", so a running meeting keeps its reminder.

Two e2e fixtures hardcoded 2026-06-10 deadlines that had aged three months into
the past; they were made relative, because the sweep correctly retires them.

### Telegram + WhatsApp auto-deploy (2026-09-18)

| Command | Result |
| ------- | ------ |
| `uv run pytest tests/unit/test_telegram_provisioner.py -q` | 21 passed |
| `uv run pytest tests/unit/test_whatsapp_provisioner.py -q` | 16 passed |
| `uv run pytest tests/integration/test_plugins.py -q` | passed (bundled registration asserts both new notifier+provisioner pairs) |
| `make check` | see below |

Both plugins register a NOTIFIER **and** a GATEWAY_PROVISIONER, so the existing
registry-driven guided setup (`#entry-form-next` → `GatewayGuideModal`) picks
them up with no flow change. Verified against the real world, not only fakes:

- Telegram: token validation via `getMe`, in-process `getUpdates` long-poll,
  bot's own messages and non-text updates ignored, the exact
  `{text,sender,chat_id,chat_type,provider,instance_id}` payload posted to
  `bot_server`, every reply page sent back in order.
- WhatsApp: real `npm install` (118 packages) by `install()`, real `start()`
  launching the Node bridge, `/health` → `{"logged_in": false,
  "status": "pending"}` then `/qr` → a genuine 4808-byte PNG QR
  (`status=scanning`), a second `start()` reusing the running bridge instead of
  launching another, and `stop()` reporting `stopped`. Smoke artifacts removed
  afterwards.
