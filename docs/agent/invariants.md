...
31. The English pack contains English text: a translated string in `en.json`
    is a bug, not a shortcut (test-enforced).
32. A key is defined exactly once. Nested objects are the style; a literal
    dotted key that flattens onto an existing nested path is a silent
    duplicate where iteration order picks the winner (test-enforced). The
    `config.desc` block is the one exception: its keys *are* dotted config
    paths, and they must not collide either.
33. Every configurable option has a `config.desc.<dotted.key>` entry in both
    built-in packs, generated by `tools/gen_option_descriptions.py`
    (test-enforced).

## Docs

34. Behavioral changes update the relevant `docs/architecture` or
    `docs/agent` page in the same change. No `docs/ai` directory.
35. Docs name real things: a plugin id, component id, module path, event name,
    `make` target or service method mentioned in a doc must exist in the code.
    `docs/build-log/BUILD_LOG.md` is exempt — it is a historical record and
    may describe things that were later removed.
