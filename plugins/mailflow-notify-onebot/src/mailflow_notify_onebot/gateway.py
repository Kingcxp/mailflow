...
class NapCatProvisioner:
    ...
    async def qr(self, instance_id: str) -> str:
        ...
        # refreshing the QR file. If the file has not changed for a long
        # while AND the gateway answers on some port, treat it as logged
        # in (a stale QR would have been regenerated by NapCat).
        try:
        ...
        if age > 90.0:
            try:
                async with httpx.AsyncClient(timeout=5.0) as client:
                    for endpoint in endpoints:
                        try:
                            resp = await client.post(f"{endpoint}/get_login_info", json={})
                            if resp.status_code < 500:
                                logger.info(
                                    "napcat %s: QR stable %ds + HTTP reachable on %s — logged in",
                                    instance_id,
                                    int(age),
                                    endpoint,
                                )
