...
- **Logs tab shows chat activity by default** — the viewer defaulted to
  ...
  `[邮件]`, `[AI 分析]`, ...) instead of the raw logger name; chat lines
  additionally carry a per-platform tag with its own color
  (`[聊天机器人·napcat]` red / `·wechaty` green / `·openwechat` amber),
  the message is indented past the tag, and duplicate-mail skips were
  demoted to DEBUG so routine noise no longer buries the chat and
  analysis lines.
